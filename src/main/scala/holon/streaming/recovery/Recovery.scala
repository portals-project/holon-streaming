...
import holon.utils.{Logger, LogConsumerRecords, LogProducerRecords, LogConsumer}
import holon.streaming.output.OutputCollectorImpl
import holon.streaming.processing.ProcFun
import holon.core.Utils.*
import holon.streaming.checkpointing.{CloudStorageCheckpointManager, DecentralizedCheckpointManager}
import holon.streaming.messaging.messages.*
import holon.core.Config.*
import holon.utils.ConsumerProducerSetup
import holon.streaming.output.LagManager
...
class Recovery(nodeId: Int) {
    ...
    private val queue = new ConcurrentLinkedQueue[holon.core.Job]()
    private val producers = scala.collection.mutable.Map.empty[Byte, holon.utils.LogProducer]
    ...
    private var procFunFactory: holon.streaming.processing.ProcFunFactory = null
    private var procFunctionPerPartition = scala.collection.mutable.Map.empty[Int, holon.streaming.processing.ProcFun]
    ...
    def submitOrUpdate(job: holon.core.Job): Unit = {
        this.queue.add(job)
    }
    ...
    private def setup(job: holon.core.Job): Unit = {
                ...
                for (rec <- records) {
                    ...
                    message match {
                        case CRDTUpdate(update, senderId, lag) =>
                            if (senderId != nodeId) {
                                logger.debug(s"Node $nodeId received holon.crdt.CRDT update from node $senderId")
                                LagManager.updateLag(senderId, lag)
                            }
                            logger.debug(s"Node $nodeId received holon.crdt.CRDT update from node $senderId, one of the $message records")
