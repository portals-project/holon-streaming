...
object NexmarkProducer {
    ...
    def runProducer(kafkaHost: String, kafkaPort: Int, sleepTime: Int): Unit = {
        ...
        logger.debug("Starting holon.examples.nexmark.data.Nexmark Producer with number of partitions: " + nrOfKafkaPartitions())
            ...
            for i <- 0 until nrOfKafkaPartitions() do
                ...
                val batch = (0L until batchSize)
                  ...
                  .map { x =>
                      ...
                      (writeBinary(i), holon.examples.nexmark.data.Nexmark.serialize(x))
                  }
