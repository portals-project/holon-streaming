...
import holon.utils.*
import holon.streaming.messaging.KafkaLogProducer
...
//        while !holon.streaming.cloud.FirestoreClient.isStartFlagSet do
...
//            Thread.sleep(1000)
    ...
    def runProducer(kafkaHost: String, kafkaPort: Int, sleepTime: Int): Unit = {
        ...
        val fileIterator = taxiEventIterator("/Users/rvang/Documents/Github/holon-streaming-clone/taxi-data", 1)

        val inputEventsPerWindow = mutable.Map.empty[Long, Long]

        while fileIterator.hasNext do
            for i <- 0 until nrOfKafkaPartitions() do
                val batch = (0 until PRODUCER_BATCH_SIZE.toInt)
                    .flatMap(_ =>
