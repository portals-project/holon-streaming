...
// Pass through
class Q2ProcessFun(partition: Int) extends ProcFun {
        ...
        if chn == CHN_INPUT then
            for record <- recs do
                ...
                if scala.util.Random.nextInt(400) == 1 then
                    ...
                    val deserializedEvent = TaxiProducer.deserialize(record._2)
                    logger.info(s"[LagAppendInput] - eventId: $recordId, timestamp: $logAppendTs, event: ${deserializedEvent.event}")
